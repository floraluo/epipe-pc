+(function($){
	'use strict';
	// LINKAGE PUBLIC CLASS DEFINITION
	// ===============================
	...
	var Linkage = function(element , options) {
		...
		this.$tabvalue  = null
		// this.inState    = null
		this.init('linkage' , element ,options)
	}
	...
	Linkage.DEFAULTS = {
		...
		dist: null,
		trigger: 'click',
		// template: '<div class="linkage-wrap"><div class="linkage" role="linkage"><div class="linkage-tab"></div><div class="linkage-content"></div></div></div>',
	...
	Linkage.prototype.init = function (type, element, options) {
		this.enabled   = true
		this.type      = type
		this.$element  = $(element)
		this.options   = this.getOptions(options)
		this.$input    = this.$element.find('.linkage-input')
		this.$content  = this.$element.find('.linkage-content')
		this.$panel    = this.$element.find('.linkage-content>div')
		this.$tabvalue = this.$element.find('.value')

		this.$input.on('click.fl.linkage', $.proxy(this.open , this))
		this.setPanel()
		this.setTab()

	}
	Linkage.prototype.setProv = function() {
		var html = "", options = this.options
		$.each( this.options.url , function(i , prov) {
			html+= '<li data-index='+i+' data-value='+prov.p+'>'+ options.formatProvResult(prov).template+'</li>'
		})
		this.$panel.eq(0).html("<ul class='area-list'>"+html+"</ul>");
		this.setCity()
	}

	Linkage.prototype.setTab = function () {
		var prov_tab = this.options.prov ? this.options.prov : this.options.url[0].p,
				city_tab = this.options.city ? this.options.city : this.options.url[0].c[0].n
		 this.$tabvalue.eq(0).html(prov_tab)
		 this.$tabvalue.eq(1).html(city_tab)

		if(!typeof(this.options.url[0].c[0].a) == 'undefined'){
			dist_tab = this.options.dist ? this.options.dist : this.options.url[0].c[0].a[0].s
			 this.$tabvalue.eq(2).html(dist_tab)
		}
	}

	Linkage.prototype.setCity = function(index) {
		var html = "", options = this.options,
				prov_index = index ? index : 0

		if(prov_index < 0||typeof(this.options.url[prov_index].c)=="undefined") return
			
		$.each( this.options.url[prov_index].c,function(i,city){
			html+="<li data-index='"+i+"' data-value='"+city.n+"'>"+ options.formatCityResult(city).template +"</li>"
		})
		this.$panel.eq(1).html("<ul class='area-list'>"+html+"</ul>")
		this.$content.find('.active li')
			.on('click.fl.linkage',  $.proxy(this.selectValue, this))
	}

	Linkage.prototype.setDist = function(index) {
		var html = "", options = this.options,
				prov_index = index ? this.$tabvalue.eq(0).attr('data-index') : 0,
				city_index = index ? index : 0

		if(city_index < 0||typeof(this.options.url[prov_index].c[city_index].a)=="undefined") return
			
		$.each( this.options.url[prov_index].c[city_index].a,function(i,dist){
			html+="<li data-index='"+i+"' data-value='"+dist.s+"'>"+ options.formatDistResult(dist).template +"</li>"
		})
		this.$panel.eq(2).html("<ul class='area-list'>"+html+"</ul>")
		this.$content.find('.active li')
			.on('click.fl.linkage',  $.proxy(this.selectValue, this))
	}

	Linkage.prototype.open = function(e) {
		e.stopPropagation()
		this.$input.val("")
		$('.linkage.active').removeClass('active')
		this.$element.find('.linkage').addClass('active')
		this.$tabvalue.eq(0).trigger('click')
		this.$content.find('li')
			.on('click.fl.linkage',  $.proxy(this.selectValue, this))
	}

	Linkage.prototype.close = function(e) {
		if($(e.target).closest(".linkage").length<=0){
			$(".linkage.active").removeClass("active")		
		}
	}
	Linkage.prototype.getValue = function(e) {
		var target = e.target,
				$this = $(target),
				$li = target.nodeName == "LI" ? $this : $this.closest('li'),
				value = $li.attr("data-value"),
				index = $li.attr("data-index"),
				panelIndex = $li.closest('div').index();
		return {
			value: value,
			index: index,
	...
	Linkage.prototype.setPanel = function(num , index) {
		// 第几级，当前index
		if( !num || num < 0 ) {
			this.setProv()
			this.setCity()
	...
	Linkage.prototype.selectValue = function (e) {
		var $nextTab , inputValue='' , i =0 , 
				thisObj = this.getValue(e),
				panelIndex = thisObj.panelIndex,
				tabCount = this.$tabvalue.length

		this.$tabvalue.eq(panelIndex).html(thisObj.value).attr('data-index',thisObj.index)
		if( panelIndex < tabCount-1 ) { 
			$nextTab = this.$tabvalue.eq(thisObj.panelIndex+1)
			if(panelIndex == 0) $nextTab.html(this.options.url[thisObj.index].c[0].n)
			if(panelIndex == 1) {
				$nextTab.html(this.options.url[this.$tabvalue.eq(0).attr('data-index')].c[thisObj.index].a[0].s)				
			}
			$nextTab.trigger('click')
			this.setPanel(panelIndex+1 , thisObj.index)
		}else if (panelIndex == tabCount-1) {
			while( i <= panelIndex) {
				inputValue += this.$tabvalue.eq(i).html()
				i++
			}
			this.$input.val(inputValue)
			this.$element.find('.linkage.active').removeClass('active')
		}
	}
	Linkage.prototype.getDefaults = function () {
		return Linkage.DEFAULTS
	}
	Linkage.prototype.getOptions = function (options) {
		options = $.extend({}, this.getDefaults(), this.$element.data(), options)

		return options
	}

	// LINKAGE PLUGIN DEFINITION
	// =========================

	function Plugin(option) {
	  return this.each(function () {
	    var $this   = $(this)
	    var data    = $this.data('fl.linkage')
	    var options = typeof option == 'object' && option

	    if (!data && /destroy|hide/.test(option)) return
	    if (!data) $this.data('fl.linkage', (data = new Linkage(this, options)))
	...
	$.fn.linkage             = Plugin
	$.fn.linkage.Constructor = Linkage


	// LINKAGE NO CONFLICT
	// ===================

	$.fn.linkage.noConflict = function () {
	  $.fn.linkage = old
	  return this
	}
